...
export const ModuleItem = ({ module, isLastModule }: { module: IModule, isLastModule: boolean }) => {
    const { selectedModule, setSelectedModule } = useSelectionStore();
    const isSelectedModule = selectedModule?.id === module.id;

    const handleSelectModule = (event: React.MouseEvent<HTMLButtonElement>) => {
        event.stopPropagation();
        setSelectedModule(isSelectedModule ? null : module);
    }
    ...
    return (
        <li className={`w-full pl-4 ${isLastModule ? "mb-4" : ""}`}>
            <button
                ...
                group hover:bg-primary/10 ${isSelectedModule ? "text-primary font-medium" : "text-neutral-600"}`}
                onClick={handleSelectModule}
            >
                <div
                    className={`h-6 w-6 p-1 rounded-md transition-all duration-200 group-hover:bg-buttons group-hover:text-white
                    ${isSelectedModule ? "bg-buttons text-white" : "bg-neutral-100 text-neutral-600"}`}
                >
                    <DataModuleIcon />
                </div>
                {module.name}
